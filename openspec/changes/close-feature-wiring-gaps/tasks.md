...
- [ ] 1.1 Session recording: call `feedRecording(sessionId, data)` from the shell `stream.on('data')` handler in `ssh.ipc.ts` so `.cast` files contain real asciicast events; verify a recorded file plays in `asciinema`.
- [ ] 1.2 Pre/post-connection hooks: execute stored `execCommands` (phase `pre` on connect, `post` on disconnect) via `macro-executor.executeExecCommands` from `ssh.ipc.ts`, honoring `ask: true` with a renderer confirmation prompt; audit-log each execution.
- [ ] 1.3 Tab reattach: subscribe to `window.onTabReattached` in `App.tsx`, recreate the tab (title, connectionId, terminal id) and reclaim ownership; verify detach → reattach round-trip keeps the live session.
- [ ] 1.4 Find in Terminal: wire the search bar to the already-loaded `SearchAddon` (`findNext`/`findPrevious`, highlight, Esc clears); listener on the pane element for `terminal:search`/`terminal:search-next`.
- [ ] 1.5 Clear/Reset/Save-log: add pane-element listeners for `terminal:clear`, `terminal:reset`, `terminal:save-log` in `XTerminal`/`useTerminal`; Save-log calls `system.startLogging` with the connection's `logPattern`.
- [ ] 1.6 Secret redaction: persist the toggle in preferences (load on startup) instead of in-memory module state.
- [ ] 1.7 Dynamic (SOCKS) tunnels: implement a per-tunnel SOCKS5 listener backed by `client.forwardOut`, or hide the Dynamic option in TunnelManager until it exists — no stub messages.
- [ ] 1.8 Run typecheck/lint/tests; update README EN/ES: remove resolved items from "Known limitations".
