# Remote Session Aliases

## Purpose

Persist a renamed tab's alias on the remote host alongside the multiplexer session it is
attached to, so the name the user gave a terminal survives app restarts and is restored
when the session is reattached from any device.
