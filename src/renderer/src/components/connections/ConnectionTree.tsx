  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === 'e' && onEdit) onEdit(node.id)
      }}
    >
      {isSelected && (
      ...
      {onEdit && (
        <button
          className="ml-auto opacity-0 group-hover:opacity-100 text-[#c7c4d7]/50 hover:text-[#e6e1e5] transition-opacity p-0.5"
          onClick={(e) => {
            e.stopPropagation()
            onEdit(node.id)
          }}
      ...
      {/* Status dot */}
      {!onEdit && (
        <span className="ml-auto w-1.5 h-1.5 rounded-full shrink-0 bg-[#c7c4d7]/20" />
      )}
      {onEdit && (
...
export function ConnectionTree({ onConnect, onEdit }: ConnectionTreeProps): JSX.Element {
